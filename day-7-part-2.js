...
addLimitFoldersSize(directories);
dirSizes.sort(function (a, b) {
  return a - b;
});
const unusedSpace = totalAvailableSpace - directoryStructure.totalSize;
const spaceToDelete = spaceNeeded - unusedSpace;

for (let i = 0; i < dirSizes.length; i++) {
  let space = dirSizes[i];
  if (space + unusedSpace >= spaceNeeded) {
    console.log(space);
    break;
  }
}
