...
const rucksacks = smallInput.split(/\n/);
let totalPriority = 0;
rucksacks.forEach((rucksack) => {
  for (let i = 0; i < rucksack.length / 2; i++) {
    const item = rucksack[i];
    if (
      rucksack.substr(rucksack.length / 2, rucksack.length / 2).includes(item)
    ) {
      totalPriority = totalPriority + priority.indexOf(item);
      break;
    }
  }
});
...
for (let i = 0; i <= rucksacks.length + 1 / 3; i = i + 3) {
  const rucksack = rucksacks[i].split('');
  rucksack.every((item) => {
    if (rucksacks[i + 1].includes(item) && rucksacks[i + 2].includes(item)) {
      secondPriority = secondPriority + priority.indexOf(item);
      return false;
    }
    return true;
  });
